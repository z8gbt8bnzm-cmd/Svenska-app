...
const WORDS = {
  ...
  familj: [
    ...
    {sv:"hund",ru:"собака",emoji:"🐶"},{sv:"katt",ru:"кот",emoji:"🐱"},
  ],
  farger: [
...
// Quiz: show Russian word, child taps/says the correct Swedish word
...
export default function SvenskaQuiz() {
  ...
  const [mode, setMode]             = useState("titta"); // titta | listen | quiz
  ...
  const startTheme = useCallback(async(t,selectedMode)=>{
    ...
    if(selectedMode==="titta"){
      // Listen & Look — just show and speak, no interaction needed
      setScreen("titta");
  ...
  // ── TITTA: go to next word and speak it
  ...
  // ── TITTA: replay current word
  const tittaSpeak = useCallback(async()=>{
    const w=wordsRef.current[wordIdxRef.current]; if(!w) return;
    await speakOut(w.sv,"sv-SE",0.68,1.0);
  },[]);
  ...
  // Quiz mic: child sees Russian word, says Swedish word
  ...
  // Quiz tap: child taps the correct SWEDISH word
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1b4b 0%,#1a3a6b 40%,#0d4a3a 100%)",fontFamily:"'Fredoka One',cursive",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"16px",position:"relative",overflow:"hidden"}}>
      ...
      {screen==="home"&&(
        <div className="card" style={{textAlign:"center",maxWidth:500,width:"100%"}}>
          ...
          <button onClick={()=>setScreen("forest")} style={{display:"inline-flex",alignItems:"center",gap:8,margin:"8px 0 18px",background:"rgba(72,199,116,0.15)",border:"1px solid rgba(72,199,116,0.4)",borderRadius:30,padding:"6px 16px",cursor:"pointer"}}>
            <span style={{fontSize:"1.2rem"}}>🌲</span>
            <span style={{color:"#48c774",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.82rem"}}>Björns skogsväg: {totalSteps} / {TOTAL_STEPS} steg</span>
          </button>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:20}}>
            {MODES.map(m=>(
              <button key={m.val} onClick={()=>setMode(m.val)} style={{
                padding:"10px 10px",borderRadius:20,border:"2px solid",
                ...
                display:"flex",flexDirection:"column",alignItems:"center",gap:2,flex:1,maxWidth:130,
              }}>
                <span>{m.label}</span>
                <span style={{fontSize:"0.65rem",opacity:0.8}}>{m.sub}</span>
              </button>
            ))}
          </div>

          <p style={{color:"#7eb8f7",fontSize:"0.85rem",marginBottom:12,fontFamily:"'Nunito',sans-serif"}}>Välj ett ämne / Выбери тему:</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {THEMES.map(t=>(
              <button key={t.id} className="tbtn" onClick={()=>startTheme(t,mode)} style={{padding:"14px 10px",borderRadius:20,border:"2px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.07)",backdropFilter:"blur(8px)",color:"white",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                ...
                {(progress[t.id]||0)>0&&<span style={{fontSize:"0.7rem",color:"#48c774",fontFamily:"'Nunito',sans-serif",fontWeight:800}}>✓ {progress[t.id]} steg</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      {screen==="titta"&&w&&(
        <div className="card" style={{maxWidth:440,width:"100%",textAlign:"center"}}>
          ...
          <div style={{marginBottom:10}}>
            <div className={bjornTalking?"bjorn-talk":bjornMood==="happy"?"bjorn-happy":""} style={{fontSize:"4.5rem",display:"inline-block",filter:"drop-shadow(0 4px 12px rgba(0,0,0,0.3))"}}>
              {bjornMood==="happy"?"🥳":"🐻"}
            </div>
          ...
          <div style={{background:"rgba(255,255,255,0.09)",borderRadius:26,padding:"28px 18px",marginBottom:18,border:"1px solid rgba(255,255,255,0.15)",backdropFilter:"blur(12px)"}}>
            ...
            <div style={{color:"#FFD700",fontSize:"3rem",fontWeight:700,marginBottom:8,letterSpacing:1}}>{w.sv}</div>
            <div style={{color:"rgba(255,255,255,0.4)",fontSize:"1rem",margin:"0 0 4px",fontFamily:"'Nunito',sans-serif"}}>—</div>
            <div style={{color:"#a0d4ff",fontSize:"1.5rem",fontFamily:"'Nunito',sans-serif",fontWeight:800}}>{w.ru}</div>
          </div>

          <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:14}}>
            <button onClick={tittaSpeak} style={{padding:"12px 20px",borderRadius:50,border:"2px solid #FFD700",background:"rgba(255,215,0,0.15)",color:"#FFD700",fontFamily:"'Nunito',sans-serif",fontWeight:800,cursor:"pointer",fontSize:"0.9rem"}}>
              🔊 Lyssna igen
            </button>
            {wordIndex+1<words.length?(
              <button onClick={tittaNext} style={{padding:"12px 20px",borderRadius:50,border:"none",background:"linear-gradient(135deg,#FFD700,#ff9500)",color:"#1a1a2e",fontFamily:"'Nunito',sans-serif",fontWeight:800,cursor:"pointer",fontSize:"0.9rem"}}>
                Nästa →
              </button>
            ):(
              <button onClick={()=>setScreen("result")} style={{padding:"12px 20px",borderRadius:50,border:"none",background:"linear-gradient(135deg,#48c774,#00a854)",color:"white",fontFamily:"'Nunito',sans-serif",fontWeight:800,cursor:"pointer",fontSize:"0.9rem"}}>
                Klart! ✓
              </button>
            )}
          </div>
      ...
      {screen==="listen"&&w&&(
        <div className={`card ${shake?"shake":""} ${bounce?"bounce":""}`} style={{maxWidth:440,width:"100%",textAlign:"center"}}>
          ...
          <div style={{marginBottom:10}}>
            <div className={bjornTalking?"bjorn-talk":bjornMood==="great"?"bjorn-happy":""} style={{fontSize:"4.5rem",display:"inline-block",filter:"drop-shadow(0 4px 12px rgba(0,0,0,0.3))"}}>
              {bjornMood==="great"?"🥳":"🐻"}
            </div>
            <div style={{background:"rgba(255,255,255,0.1)",borderRadius:14,padding:"7px 14px",margin:"6px auto 0",maxWidth:280,border:"1px solid rgba(255,255,255,0.15)"}}>
              <div style={{color:"white",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.88rem"}}>{bjornMsg.sv}</div>
          ...
          <div style={{background:"rgba(255,255,255,0.09)",borderRadius:26,padding:"24px 18px",marginBottom:14,border:"1px solid rgba(255,255,255,0.15)",backdropFilter:"blur(12px)"}}>
            ...
            <button onClick={speakWord} style={{marginTop:10,background:"rgba(255,215,0,0.15)",border:"2px solid #FFD700",borderRadius:50,padding:"7px 18px",color:"#FFD700",fontFamily:"'Nunito',sans-serif",fontWeight:800,cursor:"pointer",fontSize:"0.8rem"}}>🔊 Lyssna igen / Слушать снова</button>
          </div>
          {!showTapBtns?(
            <div style={{textAlign:"center"}}>
              ...
              {retryCount>0&&micState!=="correct"&&<button onClick={skipWord} style={{marginTop:6,background:"transparent",border:"none",color:"#7eb8f7",fontFamily:"'Nunito',sans-serif",cursor:"pointer",fontSize:"0.78rem"}}>Skip / Пропустить →</button>}
            </div>
          ):(
            <div>
              <p style={{color:"#FFD700",fontFamily:"'Nunito',sans-serif",fontSize:"0.8rem",marginBottom:10}}>Tap the Swedish word! / Нажми шведское слово!</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[w.sv,...words.filter((_,i)=>i!==wordIndex).sort(()=>Math.random()-0.5).slice(0,3).map(x=>x.sv)].sort(()=>Math.random()-0.5).map((opt,i)=>(
                  ...
                  }} style={{padding:"14px 8px",borderRadius:16,border:"2px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.08)",color:"white",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.95rem",cursor:"pointer",backdropFilter:"blur(8px)",animationDelay:`${i*0.07}s`}}>{opt}</button>
                ))}
              </div>
            </div>
          )}
          <button onClick={()=>setScreen("home")} style={{marginTop:14,background:"transparent",border:"none",color:"#7eb8f7",fontFamily:"'Nunito',sans-serif",cursor:"pointer",fontSize:"0.8rem"}}>← Hem / Домой</button>
        </div>
      )}
      ...
      {screen==="quiz"&&q&&(
        <div className={`card ${shake?"shake":""}`} style={{maxWidth:460,width:"100%",textAlign:"center"}}>
          ...
          <div style={{marginBottom:10}}>
            <div className={bjornTalking?"bjorn-talk":bjornMood==="great"?"bjorn-happy":""} style={{fontSize:"3.8rem",display:"inline-block"}}>
              {bjornMood==="great"?"🥳":"🐻"}
            </div>
          ...
          {/* Show RUSSIAN word — child must say/tap the SWEDISH word */}
          <div style={{background:"rgba(255,255,255,0.08)",borderRadius:22,padding:"18px 16px",marginBottom:12,border:"1px solid rgba(255,255,255,0.12)",backdropFilter:"blur(10px)"}}>
            ...
            <div style={{color:"#a0d4ff",fontSize:"2rem",fontFamily:"'Nunito',sans-serif",fontWeight:800,marginBottom:6}}>{q.ru}</div>
            <div style={{color:"rgba(255,255,255,0.5)",fontFamily:"'Nunito',sans-serif",fontSize:"0.8rem"}}>
              Hur säger man på svenska? / Как по-шведски?
            </div>
          </div>
          ...
          {!showTapBtns&&(
            <div style={{textAlign:"center",marginBottom:10}}>
              ...
              {retryCount>0&&micState!=="correct"&&<button onClick={()=>setShowTapBtns(true)} style={{marginTop:4,background:"transparent",border:"none",color:"#7eb8f7",fontFamily:"'Nunito',sans-serif",cursor:"pointer",fontSize:"0.76rem"}}>Use buttons / Использовать кнопки</button>}
            </div>
          )}

          {/* Tap buttons show SWEDISH options */}
          {showTapBtns&&(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {q.options.map((opt,i)=>{
                ...
                return(
                  <button key={i} className="opt pop" disabled={selected!==null} onClick={()=>handleTap(opt)} style={{padding:"14px 8px",borderRadius:16,border:`2px solid ${bc}`,background:bg,color:"white",fontFamily:"'Nunito',sans-serif",fontWeight:700,fontSize:"0.95rem",cursor:selected!==null?"default":"pointer",backdropFilter:"blur(8px)",animationDelay:`${i*0.07}s`}}>
      ...
      {screen==="result"&&(
        <div className="card" style={{textAlign:"center",maxWidth:420,width:"100%"}}>
          ...
          <h2 style={{color:"#FFD700",fontSize:"1.9rem",margin:"0 0 4px"}}>{score>=totalAnswered*0.8?"Fantastiskt!":"Bra jobbat!"}</h2>
          {mode!=="titta"&&(
          ...
          {mode==="titta"&&(
            <p style={{color:"#a0c4ff",fontFamily:"'Nunito',sans-serif",fontSize:"0.95rem",margin:"10px 0 16px"}}>Du har sett alla ord! / Ты посмотрел все слова! 🎉</p>
          )}
          {sessionCollected.length>0&&(
            <div style={{background:"rgba(72,199,116,0.1)",border:"1px solid rgba(72,199,116,0.3)",borderRadius:16,padding:"12px",marginBottom:14}}>
              <div style={{color:"#48c774",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.85rem",marginBottom:6}}>🎒 Björn hittade / Бьёрн нашёл:</div>
          ...
          <div style={{background:"rgba(255,255,255,0.06)",borderRadius:16,padding:"10px 14px",marginBottom:16}}>
            <div style={{color:"#48c774",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.82rem",marginBottom:6}}>🌲 Björns skogsväg: {totalSteps} / {TOTAL_STEPS} steg</div>
            ...
            <button onClick={()=>setScreen("forest")} style={{marginTop:8,background:"transparent",border:"none",color:"#48c774",fontFamily:"'Nunito',sans-serif",cursor:"pointer",fontSize:"0.78rem",fontWeight:800}}>Se skogen → / Посмотреть лес →</button>
          </div>
          <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={()=>startTheme(theme,mode)} style={{padding:"12px 20px",borderRadius:50,border:"none",background:"linear-gradient(135deg,#FFD700,#ff9500)",color:"#1a1a2e",fontFamily:"'Nunito',sans-serif",fontWeight:800,fontSize:"0.9rem",cursor:"pointer"}}>🔄 Igen! / Ещё раз!</button>
            <button onClick={()=>setScreen("home")} style={{padding:"12px 20px",borderRadius:50,border:"2px solid #4a6fa5",background:"transparent",color:"#a0c4ff",fontFamily:"'Nunito',sans-serif",fontWeight:700,fontSize:"0.9rem",cursor:"pointer"}}>🏠 Hem / Домой</button>
          </div>
        </div>
      )}
    </div>
